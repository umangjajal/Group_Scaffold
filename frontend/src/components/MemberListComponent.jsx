      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        {displayedMembers.map((m) => {
          ...
          return (
            <div
              key={m._id}
              className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[#2d2d2d] group transition-all cursor-default"
            >
              <div className="relative flex-shrink-0">
                ...
                <div
                  className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-[#1e1e1e] ${isOnline ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]' : 'bg-gray-600'}`}
                />
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  {m.role === 'owner' && (
                    <CheckBadgeIcon className="w-3.5 h-3.5 text-yellow-500" title="Room Owner" />
                  )}
                </div>
