...
export default function RoomCard({ room, onJoin, isMember }) {
  ...
  return (
    <article className="group relative flex flex-col bg-white rounded-3xl border border-slate-200 overflow-hidden transition-all duration-300 hover:shadow-2xl hover:shadow-blue-500/10 hover:-translate-y-1">
      {/* Visual Banner */}
      <div className={`h-24 w-full transition-colors duration-500 ${isPrivate ? 'bg-slate-900' : 'bg-blue-600 group-hover:bg-blue-500'}`}>
        <div className="flex justify-end p-4">
           <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest backdrop-blur-md border ${
             isPrivate ? 'bg-white/10 text-white border-white/20' : 'bg-blue-400/20 text-white border-blue-300/30'
           }`}>
        ...
        <div className="flex-1">
          ...
          {isPrivate && room.roomCode && (
            <div className="mb-4 p-3 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-between">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Entry Code</span>
              <span className="text-sm font-mono font-bold text-slate-700 tracking-widest">{room.roomCode}</span>
            </div>
          )}
        </div>
