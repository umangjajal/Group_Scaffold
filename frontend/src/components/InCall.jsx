...
export default function InCall({ peerId, type = 'audio', onEnd }) {
  ...
  React.useEffect(() => {
    ...
    const pc = new RTCPeerConnection({
      iceServers: [{ urls: 'stun:stun.l.google.com:19302' }, ...turnUrl],
    });
    pcRef.current = pc;
    pc.ontrack = (e) => {
    ...
    pc.onicecandidate = (e) => {
      if (e.candidate) socket.emit('call:candidate', { to: peerId, candidate: e.candidate });
    };
    ...
    (async () => {
      try {
        ...
        alert('Could not get microphone/camera: ' + e.message);
        onEnd();
      }
    })();
    return () => {
  ...
  return (
    <div style={{ border: '1px solid #bbb', padding: 10 }}>
      ...
      <div style={{ marginTop: 10 }}>
        <button
          onClick={() => {
            socket.emit('call:end', { sessionId: 'demo', reason: 'user_end' });
            onEnd();
          }}
        >
          End Call
