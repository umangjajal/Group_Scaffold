    ...
    >
      <div className="container-main flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 group" onClick={() => setIsOpen(false)}>
          ...
          <span
            className={`text-xl font-black tracking-tight ${!scrolled && isHome ? 'text-blue-900' : 'text-gray-900'}`}
          >
        ...
        <div className="hidden md:flex items-center gap-6">
          {user ? (
            <>
              <Link
                to="/groups"
                className="text-sm font-bold text-gray-600 hover:text-blue-600 flex items-center gap-1.5 transition-colors"
              >
            ...
            <>
              ...
              <Link
                to="/signup"
                className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-xl shadow-lg shadow-blue-500/20 hover:shadow-blue-500/40 transition-all"
              >
                Create Account
              </Link>
            </>
        ...
        <div className="md:hidden absolute top-full left-0 right-0 bg-white border-b border-gray-100 p-4 flex flex-col gap-3 shadow-xl animate-in fade-in slide-in-from-top-4">
          {user ? (
            <>
              ...
              <button
                onClick={handleLogout}
                className="flex items-center gap-3 p-3 rounded-lg hover:bg-red-50 text-red-500 font-bold"
              >
            ...
            <>
              <Link
                to="/login"
                onClick={() => setIsOpen(false)}
                className="p-3 text-center text-gray-700 font-bold border border-gray-100 rounded-lg"
              >
                Sign In
              </Link>
              <Link
                to="/signup"
                onClick={() => setIsOpen(false)}
                className="p-3 text-center bg-blue-600 text-white font-bold rounded-lg shadow-lg"
              >
                Create Account
              </Link>
            </>
