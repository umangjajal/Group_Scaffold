...
export default function RoomDashboard() {
  ...
  if (loading) return <div className="h-screen bg-[#f8fafc] flex items-center justify-center text-blue-600 font-bold tracking-widest uppercase text-xs animate-pulse">Initializing Room Dashboard...</div>;
      ...
      >
        ...
        <section className="chat-area border-b border-gray-100 dark:border-gray-800">
              ...
              {messages.map((m, i) => {
                ...
                return (
                  <div key={m.id || i} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                    {!isMe && <span className="text-[10px] font-bold text-blue-500 mb-1 ml-1">{m.sender?.name || m.sender}</span>}
           ...
           <div className="chat-input-box">
              <form onSubmit={e => { e.preventDefault(); sendMessage(); }} className="input-wrapper">
                <input 
                  value={messageInput}
                  onChange={e => setMessageInput(e.target.value)}
                  placeholder="Type a message..."
                />
                <button type="submit" className="p-1.5 text-blue-500 hover:text-blue-600 transition-colors">
           ...
           <div className="sidebar-tabs">
              ...
              <div className="mb-8">
                ...
                <div className="flex gap-2 mb-4">
                  ...
                  />
                  <button onClick={createTask} className="p-2 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-xl transition-all">
                ...
                <div className="space-y-1">
                  {tasks.map(t => (
                    <div key={t.id} className="task-item group">
                      ...
                      <span className={`flex-1 text-xs font-medium ${t.completed ? 'text-gray-400 line-through' : ''}`}>{t.title}</span>
                      <button onClick={() => socketRef.current?.emit('task:delete', { groupId: roomId, taskId: t.id })} className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-500 transition-all">
                  ...
                  {members.map(m => (
                    <div key={m._id} className="member-item">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[9px] text-gray-500 uppercase font-black tracking-tighter">{m.role}</span>
                      </div>
                    </div>
                  ))}
                </div>
    ...
    <article className="video-card">
      {cameraOn ? (
        ...
        <div className="video-placeholder">
           ...
           <span className="text-xs md:text-sm font-bold text-gray-500 uppercase tracking-tight">{user?.name}</span>
        </div>
      )}
